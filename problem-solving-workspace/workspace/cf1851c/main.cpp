...
void solve() {
  ...
  if( n == 1 ) {
    cout << (1%k == 0 ? "YES" : "NO") << endl;  
    return;
  }

  if( c[0] == c[n-1]) {
    int same_color_tiles = 2;
    for( int i = 1; i < n -1 && same_color_tiles < k; ++ i ) 
      if( c[i] == c[0] ) same_color_tiles ++;
    cout << (same_color_tiles >= k ? "YES" : "NO") << endl;  
    return;
  }
  int index_last_c1 = 0;
  int cnt = 0;
  ...
  //debug(n,k);
  //print_vector(c);
  //debug( index_last_c1, index_first_cn);

  cout << (c1_found && cn_found && index_last_c1 < index_first_cn ? "YES" : "NO") << endl;
}
