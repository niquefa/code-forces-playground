...
void solve() {
  ...
  vector<long long> maximals(n);

  vector<long long> operations(n);
  int i_max = index_max_element(a);
  long long ans = a[i_max];
  maximals[n-1] = a[n-1];
  maximals[n-2] = a[n-2];
  for( int i = n-2 ; i >= 0 ; --i ) {
    if( maximals[i] > maximals[i+1]) {
      maximals[i] = maximals[i];
    } else {
      maximals[i] = maximals[i+1] + 1;
    }
    operations[i] = maximals[i] - a[i];
  }
  debug(i_max, ans);
  print_vector(a, "\ta");
  print_vector(maximals, "\tm");
  print_vector(operations, "\tp");

  for( int i = 0 ; i < n ; ++ i ) {
    if( maximals[i] < ans ) continue;
    for ( int t = maximals[i]; t >= ans; -- t ) {
      int ops = t - a[i];
      if( ops > k ) continue;
      if( ops == k ) {
        ans = t;
