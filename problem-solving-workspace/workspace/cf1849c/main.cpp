...
struct binary_trie_node {
  unique_ptr<binary_trie_node> children[2];
  bool binary_end_flag; 

  binary_trie_node(): binary_end_flag(false) {
    children[0] = nullptr;
    children[1] = nullptr;
  }
};

bool insert_binary_word(binary_trie_node* root, const string &key) {
  binary_trie_node* moving_trie_node = root;
  for (int i = 0; i < key.length(); i++) {
    int index = key[i] - '0';
    if (!moving_trie_node->children[index])
      moving_trie_node->children[index] = make_unique<binary_trie_node>();

    moving_trie_node = moving_trie_node->children[index].get();
  }

  if(moving_trie_node->binary_end_flag) return false; 

  moving_trie_node->binary_end_flag = true; 
  return true; 
}
...
void solve() {
  ...
  string s, tmp;
  cin >> n >> m >> s;
  unordered_set<long long> operations;
  unique_ptr<binary_trie_node> root = make_unique<binary_trie_node>();

  int answer = 0;
  long long hash_value = 0;
  for( int i = 0, a, b; i < m; ++ i ) {
    cin >> a >> b;
    hash_value = 1000000 * a + b;
    if( operations.count(hash_value) )
      continue;
    operations.insert(hash_value);
    tmp = s;
    sort(tmp.begin()+a-1, tmp.begin()+b);
    if(insert_binary_word(root.get(), tmp)) 
      answer++;
    debug(a,b,hash_value,tmp,s,answer);
  };
  cout << answer << endl;
}
